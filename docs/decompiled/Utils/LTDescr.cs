...
public class LTDescr
{
	...
	public LTDescr setColor()
	{
		...
		easeInternal = delegate
		{
			newVect = easeMethod();
			val = newVect.x;
			Color color4 = tweenColor(this, val);
			if (spriteRen != null)
			{
				spriteRen.color = color4;
				colorRecursiveSprite(trans, color4);
			}
			else if (type == TweenAction.COLOR)
			{
				colorRecursive(trans, color4, useRecursion);
			}
			if (dt != 0f && _optional.onUpdateColor != null)
			{
				_optional.onUpdateColor(color4);
			}
			else if (dt != 0f && _optional.onUpdateColorObject != null)
			{
				_optional.onUpdateColorObject(color4, _optional.onUpdateParam);
			}
		};
		return this;
	}
